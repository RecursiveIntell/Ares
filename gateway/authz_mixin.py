...
def _platform_gate_env(name: str, default: str = "") -> str:
    ...
    Under multiplex, a scoped miss returns ``default`` rather than falling
    through to process-global state. Scope-resolution failure is fail-closed
    for the same reason: another profile's first-writer env value must never
    become authorization input. Single-profile deployments keep ``os.getenv``.
    """
    if not name:
        return default
    try:
