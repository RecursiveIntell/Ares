...
def _close_agent(agent, session_db) -> None:
    """Teardown for oneshot; every step is best-effort because hard exit skips finalizers."""
    ...
    # Ares automation custody: optionally retain the transcript for audit while keeping it
    # out of the ordinary Desktop Sessions projection.
    if agent is not None and session_db is not None:
        _quietly("session archive", lambda: _archive_oneshot_session_if_requested(agent, session_db))
